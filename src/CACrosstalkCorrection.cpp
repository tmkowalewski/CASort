...
void CACrosstalkCorrection::FillXTalkHistograms(const std::array<std::shared_ptr<TH2D>, 6>& xtalkPairHists,
                                                const std::array<double, 4>& xtalE, std::array<double, 4>& xtalT)
{
...
CACrosstalkCorrection::CrosstalkFit CACrosstalkCorrection::FitCrosstalkCorrection(const TH2D* hist)
{
    ...
    auto fitFunc =
        std::make_unique<TF1>("crosstalk_fit_func", CrosstalkFitFunction, 0, kTargetEnergy + kFitWindow / 2.0, 3);
...
void CACrosstalkCorrection::WriteCrosstalkMatrices(const std::string&           fileName,
...
std::vector<std::function<std::array<double, 4>(std::array<double, 4>)>>
    CACrosstalkCorrection::MakeCorrections(const std::string& fileName)
{
