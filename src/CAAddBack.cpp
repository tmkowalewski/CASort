...
double CAAddBack::GetAddBackEnergy(std::array<double, 4> xtalE, std::array<double, 4> xtalT)
{
    ...
    double primaryE = kAddBackThreshold; // Start with threshold, if no hits are above this
                                         // then add-back will return 0
    int                   primaryIdx = -1;
    double                finalE = 0;
    ...
    else
    {
        ...
        std::cout << "[WARNING] No primary hit found for add-back. Returning 0 "
                     "energy."
                  << std::endl;
        std::cout << "xtalE = {";
