...
constexpr double kTargetEnergy =
    2598.500;                                // Energy in keV of gamma ray used for crosstalk calibration
...
void FillXTalkHistograms(const std::array<std::shared_ptr<TH2D>, 6>& xtalkPairHists,
...
void WriteCrosstalkMatrices(const std::string&           fileName,
...
std::vector<std::function<std::array<double, 4>(std::array<double, 4>)>>
    MakeCorrections(const std::string& xtalkCorrDir);
